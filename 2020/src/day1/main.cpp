...
int part2(const std::vector<int> list) {

  for (int i = 0; i < list.size(); i++) {
    for (int j = i + 1; j < list.size(); j++) {
      for (int k = j + 1; k < list.size(); k++) {
        int target = list[i] + list[j] + list[k] - 2020;
        if (target == 0) {
...
int main() {
  std::vector<int> input = {
      1227, 1065, 329,  1063, 1889, 1700, 1805, 1373, 389,  1263, 1276, 1136,
  ...
  for (int i = 0; i < input.size(); ++i) {
    output.emplace(input[i]);
    const int target = 2020 - input[i];
    // Loop back through the list and find the difference
    std::unordered_set<int>::const_iterator itr = output.find(target);
    // If the iterator is at the end then we haven't found the match
    if (itr != output.end()) {
      assert((input[i] + target) == 2020);
  ...
  if (ans2 == 0) {
    ...
    std::cout << "Final Answer: " << ans2 << std::endl;
  };
};
